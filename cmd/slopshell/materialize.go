...
const (
	materializedRootDir = ".slopshell/materialized"
	archiveManifestPath = ".slopshell/manifest.json"
)
...
func parseMaterializeArgs(args []string) (materializeConfig, int64, string, int) {
	...
	cfg := materializeConfig{dataDir: filepath.Join(os.Getenv("HOME"), ".slopshell-web")}
	...
	rest := fs.Args()
	if len(rest) != 2 {
		fmt.Fprintln(os.Stderr, "usage: slopshell materialize [--data-dir DIR] <artifact-id> <workspace-path>")
		return materializeConfig{}, 0, "", 2
	}
...
func parseArchiveArgs(args []string) (materializeConfig, string, int) {
	...
	rest := fs.Args()
	if len(rest) != 1 {
		fmt.Fprintln(os.Stderr, "usage: slopshell archive [--data-dir DIR] <workspace-path>")
		return materializeConfig{}, "", 2
	}
	return cfg, rest[0], 0
}
...
func openCLIStore(dataDir string) (*store.Store, error) {
	...
	return store.New(filepath.Join(strings.TrimSpace(dataDir), "slopshell.db"))
}
...
func materializeArtifactWithOptions(st *store.Store, artifact store.Artifact, workspacePath string, updateStore bool) (materializedArtifact, error) {
	...
	if err := os.MkdirAll(filepath.Join(workspaceDir, ".slopshell"), 0o755); err != nil {
		return materializedArtifact{}, err
	}
...
func renderCalendarICS(artifact store.Artifact, meta map[string]any) string {
	...
	b.WriteString("PRODID:-//Slopshell//Archive//EN\r\n")
	b.WriteString("BEGIN:VEVENT\r\n")
	fmt.Fprintf(&b, "UID:slopshell-artifact-%d@local\r\n", artifact.ID)
