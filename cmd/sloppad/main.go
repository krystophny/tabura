...
import (
	...
	"github.com/krystophny/sloppad/internal/ptt"
	"github.com/krystophny/sloppad/internal/serve"
	"github.com/krystophny/sloppad/internal/store"
	updater "github.com/krystophny/sloppad/internal/update"
	"github.com/krystophny/sloppad/internal/web"
)
...
func cmdMCPServer(args []string) int {
	...
	projectDir := fs.String("project-dir", ".", "project dir")
	dataDir := fs.String("data-dir", filepath.Join(os.Getenv("HOME"), ".sloppad-web"), "data dir")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	res, err := protocol.BootstrapProject(*projectDir)
	...
	st, err := store.New(filepath.Join(*dataDir, "sloppad.db"))
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
...
func parseServerConfig(args []string) (*serverConfig, int) {
	...
	cfg := &serverConfig{
		dataDir: filepath.Join(os.Getenv("HOME"), ".sloppad-web"),
	}
	projectDir := fs.String("project-dir", ".", "project dir")
	...
	fs.StringVar(&cfg.model, "model", "", "LLM model for chat (default: env SLOPPAD_APP_SERVER_MODEL or "+web.DefaultModel+")")
	fs.StringVar(&cfg.sparkReasoningEffort, "spark-reasoning-effort", "", "Spark thinking budget, e.g. low|medium|high (default: env SLOPPAD_APP_SERVER_SPARK_REASONING_EFFORT or low)")
	fs.StringVar(&cfg.ttsURL, "tts-url", "", "TTS server URL (default: env SLOPPAD_TTS_URL or "+web.DefaultTTSURL+")")
...
func cmdSetPassword(args []string) int {
	...
	dataDir := fs.String("data-dir", filepath.Join(os.Getenv("HOME"), ".local/share/sloppad-web"), "data dir")
	if err := fs.Parse(args); err != nil {
		return 2
	}
...
func cmdPTTDaemon(args []string) int {
	...
	fs.StringVar(&cfg.WebAPIURL, "web-api-url", cfg.WebAPIURL, "sloppad web API URL for STT replacements")
...
func cmdUpdate(args []string) int {
	...
	if res.Updated {
		fmt.Printf("Updated sloppad %s -> %s. Restart service to apply.\n", res.CurrentVersion, res.LatestVersion)
		return 0
	}
...
func formatVersionLine(rawVersion, rawCommit, goos, goarch string) string {
	...
	return fmt.Sprintf("sloppad %s (%s) %s/%s", release, shortCommit, goos, goarch)
}
