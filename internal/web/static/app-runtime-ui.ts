...
const MATH_SEGMENT_TOKEN_PREFIX = '@@SLOPSHELL_CHAT_MATH_SEGMENT_';
...
export function forceUiHardReload(reason = 'deployment') {
  ...
  url.searchParams.set('__slopshell_reload', Date.now().toString(36));
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    root.innerHTML = `
      <div class=\"liability-modal-card\" role=\"dialog\" aria-modal=\"true\" aria-label=\"Liability notice\">
        ...
        <p>Slopshell is provided as-is. You are solely responsible for backups, verification, and safe operation.</p>
...
export function hasVisibleRealCanvasArtifact() {
  ...
  const ref = String(state.currentCanvasArtifact?.path || state.currentCanvasArtifact?.title || '')
    ...
    .toLowerCase();
  if (!ref) return false;
  return !ref.startsWith('.slopshell/artifacts/tmp/')
    && !ref.startsWith('slopshell/artifacts/tmp/');
}
