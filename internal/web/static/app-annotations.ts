...
const ANNOTATION_STORAGE_KEY = 'slopshell.annotations.v1';
...
export function initAnnotationUi() {
  ...
  document.addEventListener('slopshell:canvas-rendered', (event) => {
    activeDescriptor = normalizeDescriptor((event as CustomEvent)?.detail);
    renderActiveAnnotations();
  });
  document.addEventListener('slopshell:canvas-cleared', () => {
