...
import {
  SLOPPAD_CIRCLE_CORNERS,
  SLOPPAD_CIRCLE_LAYOUT,
  SLOPPAD_CIRCLE_SEGMENTS,
  normalizeSloppadCircleCorner,
  sloppadCircleToolIcon,
  sloppadCircleToolIconID,
  type SloppadCircleCorner,
} from './sloppad-circle-contract.js';
...
function circleRoot() {
  const node = document.getElementById('sloppad-circle');
  return node instanceof HTMLElement ? node : null;
}

function circleDot() {
  const node = document.getElementById('sloppad-circle-dot');
  return node instanceof HTMLButtonElement ? node : null;
}

function circleMenu() {
  const node = document.getElementById('sloppad-circle-menu');
  return node instanceof HTMLElement ? node : null;
}

function cornerControlsRoot() {
  const node = document.getElementById('sloppad-circle-corner-controls');
  return node instanceof HTMLElement ? node : null;
}
...
function dotPositionForCorner(shellSize: number, dotSize: number, corner: SloppadCircleCorner) {
  const top = corner.startsWith('top_') ? 0 : shellSize - dotSize;
  const left = corner.endsWith('_left') ? 0 : shellSize - dotSize;
  return { top, left };
}

function dotCenterForCorner(shellSize: number, dotSize: number, corner: SloppadCircleCorner) {
...
function cornerSigns(corner: SloppadCircleCorner) {
  return {
...
function storageCorner() {
  try {
    return normalizeSloppadCircleCorner(window.localStorage.getItem(TOOL_PALETTE_POSITION_STORAGE_KEY) || '');
  } catch (_) {
    return 'bottom_right';
  }
}

function currentCorner(): SloppadCircleCorner {
  const current = normalizeSloppadCircleCorner(String(state.toolPalettePosition || '').trim());
...
function persistCorner(next: SloppadCircleCorner) {
...
function setCircleExpanded(next: boolean) {
  ...
  document.body.classList.toggle('sloppad-circle-expanded', circleExpanded);
  renderSloppadCircle();
}
...
export function collapseSloppadCircle() {
...
function onSegmentClick(event: Event) {
  ...
  const segment = target.closest('.sloppad-circle-segment');
...
function iconMarkup(icon: string) {
  return `<span class="sloppad-circle-icon" aria-hidden="true">${icon}</span>`;
}

function syncSegmentMarkup() {
  const menu = circleMenu();
  if (!(menu instanceof HTMLElement)) return;
  SLOPPAD_CIRCLE_SEGMENTS.forEach((segmentContract) => {
...
function ensureCornerControls() {
  ...
  SLOPPAD_CIRCLE_CORNERS.forEach((corner) => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'edge-btn edge-icon-btn sloppad-circle-corner-btn';
    ...
    button.addEventListener('click', () => {
      setSloppadCircleCorner(corner.id);
    });
...
function syncCornerControls() {
  ...
  root.querySelectorAll('button[data-corner]').forEach((node) => {
    if (!(node instanceof HTMLButtonElement)) return;
    const nextCorner = normalizeSloppadCircleCorner(node.dataset.corner || '');
...
function shellSize() {
  return window.innerWidth <= 720
    ? SLOPPAD_CIRCLE_LAYOUT.shell_size_mobile_px
    : SLOPPAD_CIRCLE_LAYOUT.shell_size_px;
}

function dotSize() {
  return window.innerWidth <= 720
    ? SLOPPAD_CIRCLE_LAYOUT.dot_size_mobile_px
    : SLOPPAD_CIRCLE_LAYOUT.dot_size_px;
}

function segmentSize() {
  return window.innerWidth <= 720
    ? SLOPPAD_CIRCLE_LAYOUT.segment_size_mobile_px
    : SLOPPAD_CIRCLE_LAYOUT.segment_size_px;
}
...
function applyCornerGeometry() {
  ...
  root.style.setProperty('--sloppad-circle-shell-size', `${nextShellSize}px`);
  root.style.setProperty('--sloppad-circle-dot-size', `${nextDotSize}px`);
  root.style.setProperty('--sloppad-circle-segment-size', `${nextSegmentSize}px`);
...
export function initSloppadCircle() {
  const root = circleRoot();
  const dot = circleDot();
  const menu = circleMenu();
  if (!(root instanceof HTMLElement) || !(dot instanceof HTMLButtonElement) || !(menu instanceof HTMLElement)) return;
...
export function setSloppadCircleCorner(next: string) {
  const normalized = normalizeSloppadCircleCorner(next);
...
export function renderSloppadCircle() {
  const root = circleRoot();
  const dot = circleDot();
  const menu = circleMenu();
  if (!(root instanceof HTMLElement) || !(dot instanceof HTMLButtonElement) || !(menu instanceof HTMLElement)) return;
  ...
  dot.dataset.icon = sloppadCircleToolIconID(tool);
  dot.dataset.session = session;
  dot.dataset.silent = String(Boolean(state.ttsSilent));
  dot.innerHTML = `${iconMarkup(sloppadCircleToolIcon(tool))}<span class="sloppad-circle-dot-badge" aria-hidden="true">${sessionLabel}</span>`;
  dot.title = circleExpanded ? 'Close Sloppad Circle' : 'Open Sloppad Circle';
  dot.setAttribute(
    'aria-label',
    `${circleExpanded ? 'Close' : 'Open'} Sloppad Circle. Live mode: ${sessionLabel}. Current tool: ${toolLabel}. Fast mode: ${state.fastMode ? 'on' : 'off'}.`,
  );
  dot.setAttribute('aria-expanded', circleExpanded ? 'true' : 'false');

  const segments = menu.querySelectorAll('.sloppad-circle-segment');
