export type SloppadCircleCorner = 'top_left' | 'top_right' | 'bottom_left' | 'bottom_right';
export type SloppadCircleSegmentKind = 'session' | 'toggle' | 'tool';
export type SloppadCircleSegmentID =
  | 'dialogue'
  | 'meeting'
  | 'silent'
  | 'fast'
...
type SloppadCircleSegmentContract = {
  id: SloppadCircleSegmentID,
  kind: SloppadCircleSegmentKind,
  icon: string,
  icon_id: string,
  label: string,
  angle_deg: number,
...
type SloppadCircleCornerOption = {
  id: SloppadCircleCorner,
  icon: string,
  label: string,
};
...
export const SLOPPAD_CIRCLE_TOOL_ICON_IDS: Record<string, string> = {
  pointer: 'arrow',
  highlight: 'marker',
  ink: 'pen_nib',
...
export const SLOPPAD_CIRCLE_TOOL_ICONS: Record<string, string> = {
...
export const SLOPPAD_CIRCLE_SEGMENTS: SloppadCircleSegmentContract[] = [
  {
    id: 'dialogue',
    kind: 'session',
    icon_id: 'dialogue',
  ...
  },
  {
    ...
    angle_deg: 70,
    radius_px: 82,
    icon: SLOPPAD_CIRCLE_TOOL_ICONS.prompt,
  },
  {
    id: 'text_note',
    kind: 'tool',
    ...
    angle_deg: 78,
    radius_px: 136,
    icon: SLOPPAD_CIRCLE_TOOL_ICONS.text_note,
  },
  {
    id: 'pointer',
    kind: 'tool',
    ...
    angle_deg: 89,
    radius_px: 217,
    icon: SLOPPAD_CIRCLE_TOOL_ICONS.pointer,
  },
  {
    id: 'highlight',
    kind: 'tool',
    ...
    angle_deg: 67,
    radius_px: 223,
    icon: SLOPPAD_CIRCLE_TOOL_ICONS.highlight,
  },
  {
    id: 'ink',
    kind: 'tool',
    ...
    angle_deg: 52,
    radius_px: 233,
    icon: SLOPPAD_CIRCLE_TOOL_ICONS.ink,
  },
];

export const SLOPPAD_CIRCLE_CORNERS: SloppadCircleCornerOption[] = [
  {
    id: 'top_left',
    label: 'Place Sloppad Circle in the top left corner',
  ...
  {
    id: 'top_right',
    label: 'Place Sloppad Circle in the top right corner',
  ...
  {
    id: 'bottom_left',
    label: 'Place Sloppad Circle in the bottom left corner',
  ...
  {
    id: 'bottom_right',
    label: 'Place Sloppad Circle in the bottom right corner',
...
export const SLOPPAD_CIRCLE_BUG_ICON = strokeIcon('<path d="M12 8.5a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7Z"/><path d="M12 4v2.2"/><path d="M6.5 6.5 8 8"/><path d="M17.5 6.5 16 8"/><path d="M5 11h2"/><path d="M17 11h2"/><path d="M6.5 15.5 8 14"/><path d="M17.5 15.5 16 14"/><path d="M9.4 18h5.2"/>');

export const SLOPPAD_CIRCLE_LAYOUT = Object.freeze({
  storage_key: 'sloppad.toolPalettePosition',
  ...
  corners: SLOPPAD_CIRCLE_CORNERS.map((corner) => corner.id),
  segments: SLOPPAD_CIRCLE_SEGMENTS.map((segment) => ({
    id: segment.id,
    kind: segment.kind,
...
export function normalizeSloppadCircleCorner(value: string): SloppadCircleCorner {
  const clean = String(value || '').trim().toLowerCase();
  return SLOPPAD_CIRCLE_CORNERS.some((corner) => corner.id === clean)
    ? clean as SloppadCircleCorner
    : 'bottom_right';
}

export function sloppadCircleToolIcon(tool: string) {
  const clean = String(tool || '').trim().toLowerCase();
  return SLOPPAD_CIRCLE_TOOL_ICONS[clean] || SLOPPAD_CIRCLE_TOOL_ICONS.pointer;
}

export function sloppadCircleToolIconID(tool: string) {
  const clean = String(tool || '').trim().toLowerCase();
  return SLOPPAD_CIRCLE_TOOL_ICON_IDS[clean] || SLOPPAD_CIRCLE_TOOL_ICON_IDS.pointer;
}
