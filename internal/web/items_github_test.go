...
func TestGitHubIssueSyncMigratesLegacyBugReportItems(t *testing.T) {
	...
	if !strings.Contains(command, "--repo "+slopshellBugReportOwnerRepo) {
		t.Fatalf("gh args = %q, want explicit repo %q", command, slopshellBugReportOwnerRepo)
	}
}
...
func TestTrackedGitHubIssueSyncUsesSourceRefRepo(t *testing.T) {
	...
	source := "github"
	sourceRef := githubIssueSourceRef(slopshellBugReportOwnerRepo, 77)
