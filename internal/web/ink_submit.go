...
func (a *App) handleInkSubmit(w http.ResponseWriter, r *http.Request) {
	...
	inkDir := filepath.Join(project.RootPath, ".slopshell", "artifacts", "ink")
	...
	fmt.Fprintf(&b, "- SVG artifact: `%s`\n\n", filepath.ToSlash(filepath.Join(".slopshell", "artifacts", "ink", svgName)))
	if pngName != "" {
		fmt.Fprintf(&b, "- PNG artifact: `%s`\n\n", filepath.ToSlash(filepath.Join(".slopshell", "artifacts", "ink", pngName)))
	}
