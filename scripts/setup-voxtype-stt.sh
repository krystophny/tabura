...
HOST="${SLOPSHELL_STT_HOST:-127.0.0.1}"
PORT="${SLOPSHELL_STT_PORT:-8427}"
LANGUAGE_RAW="${SLOPSHELL_STT_LANGUAGE:-en,de}"
THREADS="${SLOPSHELL_STT_THREADS:-4}"
PROMPT="${SLOPSHELL_STT_PROMPT:-}"
MODEL="${SLOPSHELL_STT_MODEL:-large-v3-turbo}"
# Hotkey is read from voxtype's own config (~/.config/voxtype/config.toml).
# Only override if explicitly set via SLOPSHELL_STT_HOTKEY.
HOTKEY="${SLOPSHELL_STT_HOTKEY:-}"
