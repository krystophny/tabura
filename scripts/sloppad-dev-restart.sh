...
LOCK_FILE="${XDG_RUNTIME_DIR:-/tmp}/sloppad-dev-reload.lock"
INCLUDE_PTYD="${1:-}"
...
go build -o sloppad ./cmd/sloppad
...
if [[ "$INCLUDE_PTYD" == "--include-ptyd" ]]; then
  restart_service sloppad-ptyd.service io.sloppad.ptyd
fi
restart_service sloppad-codex-app-server.service io.sloppad.codex-app-server
restart_service sloppad-llm.service io.sloppad.llm
restart_service sloppad-codex-llm.service io.sloppad.codex-llm
restart_service sloppad-web.service io.sloppad.web
