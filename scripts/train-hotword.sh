...
TRAINER_DIR="${SLOPSHELL_HOTWORD_TRAINER_DIR:-$ROOT_DIR/tools/openwakeword-trainer}"
CONFIG_PATH="${SLOPSHELL_HOTWORD_CONFIG:-$ROOT_DIR/scripts/hotword-config.yaml}"
OUTPUT_DIR="${SLOPSHELL_HOTWORD_OUTPUT_DIR:-$ROOT_DIR/models/hotword}"
PYTHON_BIN="${PYTHON:-python3}"
SKIP_PIP_INSTALL="${SLOPSHELL_HOTWORD_SKIP_PIP_INSTALL:-0}"
...
if [[ "$SKIP_PIP_INSTALL" != "1" ]]; then
  ...
  if [[ "${SLOPSHELL_HOTWORD_ORT_PACKAGE:-}" != "" ]]; then
    ORT_PACKAGE="$SLOPSHELL_HOTWORD_ORT_PACKAGE"
