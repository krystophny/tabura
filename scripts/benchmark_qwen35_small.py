...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-dir", default=os.getenv("SLOPSHELL_LLM_MODEL_DIR", str(Path.home() / ".local/share/slopshell-llm/models")))
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=18426)
    parser.add_argument("--ctx-size", type=int, default=int(os.getenv("SLOPSHELL_LLM_CTX", "16384")))
    parser.add_argument("--threads", type=int, default=int(os.getenv("SLOPSHELL_LLM_THREADS", "4")))
    parser.add_argument("--ngl", type=int, default=int(os.getenv("SLOPSHELL_LLM_NGL", "99")))
    ...
    parser.add_argument(
        "--results-json",
        default="",
        help="Optional explicit output path; default writes into .slopshell/artifacts/benchmarks/",
    )
...
def start_llama_server(args: argparse.Namespace, model_path: Path, log_file: Path) -> subprocess.Popen[Any]:
    ...
    proc._slopshell_log_handle = log_handle  # type: ignore[attr-defined]
    return proc
    ...
    if proc.poll() is None:
        ...
        try:
            ...
            try:
                ...
                pass
            proc.wait(timeout=5)
    log_handle = getattr(proc, "_slopshell_log_handle", None)
    if log_handle:
        try:
            log_handle.close()
        except Exception:
...
def default_results_path(repo_root: Path) -> Path:
    ...
    out_dir = repo_root / ".slopshell" / "artifacts" / "benchmarks"
...
def main() -> int:
    ...
    artifacts_dir = repo_root / ".slopshell" / "artifacts"
