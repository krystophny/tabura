...
# Everything goes into ~/.local/share/sloppad-piper-tts/
#
...
PIPER_DIR="${HOME}/.local/share/sloppad-piper-tts"
...
confirm_default_yes() {
    local prompt="$1"
    if [ "${SLOPPAD_ASSUME_YES:-0}" = "1" ]; then
        echo "SLOPPAD_ASSUME_YES=1 set; accepting: ${prompt}"
        return 0
    fi
...
if [ "$(uname -s)" = "Darwin" ]; then
    echo "  2. launchctl load ~/Library/LaunchAgents/io.sloppad.piper-tts.plist"
else
    echo "  2. systemctl --user start sloppad-piper-tts.service"
fi
echo "  3. Test:"
