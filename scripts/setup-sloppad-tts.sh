...
# Everything goes into ~/.local/share/sloppad-tts/
#
...
if [ -f "$REFERENCE_WAV" ]; then
    ...
    TMP_MP3="$(mktemp /tmp/sloppad-tts-ref-XXXXXX.mp3)"
...
if [ ! -f "${PREDEFINED_VOICES_DIR}/sloppad-default.wav" ]; then
    cp "$REFERENCE_WAV" "${PREDEFINED_VOICES_DIR}/sloppad-default.wav"
    echo "Copied reference voice to predefined_voices/sloppad-default.wav"
else
...
echo ""
echo "=== Sloppad TTS Setup Complete ==="
echo "  TTS dir:        $TTS_DIR"
echo "  Reference voice: $REFERENCE_WAV"
echo "  Server dir:     $SERVER_DIR"
echo ""
echo "Next steps:"
echo "  1. Run: scripts/install-sloppad-user-units.sh"
echo "  2. systemctl --user start sloppad-tts.service"
echo "  3. Test (first call downloads model weights ~1 GB):"
echo "     curl -X POST http://127.0.0.1:8423/v1/audio/speech \\"
echo "       -H 'Content-Type: application/json' \\"
echo "       -d '{\"model\":\"chatterbox\",\"input\":\"Hello\",\"voice\":\"sloppad-default\",\"response_format\":\"wav\"}' > /tmp/test.wav"
