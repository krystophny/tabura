...
Direct ink is available as an annotate-surface tool; dirty ink shows explicit submit/clear controls and saves artifacts under `.slopshell/artifacts/ink/`.
...
Slopshell exposes exactly two live session types through the hidden top edge panel:
...
- built-in `Computer` hotword
- one shared browser audio/runtime owner
- no floating on-canvas launcher

`Dialogue` is for direct back-and-forth with Slopshell:
...
- `Computer` remains a direct trigger
