...
- `--model` CLI flag, `SLOPSHELL_APP_SERVER_MODEL` env var, and hardcoded default fallback
...
- Rewrote `buildPromptFromHistory` with system instruction block: Slopshell identity, canvas action markers, guidelines
...
### Rename and Cleanup
- Complete `tabula` to `slopshell` rename across all files, systemd units, scripts
...
- Set `SLOPSHELL_APP_SERVER_MODEL` in systemd unit or use `--model` flag (defaults to `gpt-5.3-codex-spark`)
