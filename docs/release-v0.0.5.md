...
`v0.0.5` introduces Push To Prompt voice capture backed by VoxType MCP and removes the Helpy STT provider path from Sloppad.

## Highlights

- Added `sloppad voxtype-mcp` command and local VoxType MCP server (`/mcp`, `/health`).
- Added user `systemd` unit `sloppad-voxtype-mcp.service` for always-on local bridge mode.
...
- Helpy STT integration was removed from Sloppad in this release.
