...
This document formalizes the audio privacy guarantees for Slopshell's speech-to-text pipeline, including meeting capture, tap-to-talk transcription, and capture-mode voice memos.
...
The following are prohibited in all Slopshell code paths:
...
The voxtype sidecar (`slopshell-stt.service`) receives audio via HTTP multipart POST to `/v1/audio/transcriptions`. The sidecar:
...
- **Swap/page-file**: on a local install, the OS may page RAM to swap. This is outside Slopshell's control. Users who require confidentiality against swap forensics should use encrypted swap or disable swap.
