...
async function clickSegment(page: Page, segment: string) {
  await page.evaluate((name) => {
    const button = document.getElementById(`sloppad-circle-segment-${name}`);
    if (!(button instanceof HTMLButtonElement)) {
...
test('top panel keeps summary only while Sloppad Circle owns live controls', async ({ page }) => {
...
test('circle segments switch tools without using the top panel', async ({ page }) => {
  ...
  await expect(page.locator('#sloppad-circle-segment-ink')).toHaveAttribute('aria-pressed', 'true');
...
test('circle keeps live mode and tool mode visibly separate', async ({ page }) => {
  await expect(page.locator('#sloppad-circle-dot')).toHaveAttribute('aria-label', /Live mode: Manual/);
  await expect(page.locator('#sloppad-circle-dot .sloppad-circle-dot-badge')).toHaveText('Manual');

  await openCircle(page);
  await expect(page.locator('#sloppad-circle-segment-dialogue')).toHaveAttribute('aria-label', 'Live mode: Dialogue');
  await expect(page.locator('#sloppad-circle-segment-prompt')).toHaveAttribute('aria-label', 'Tool: Prompt');

  await clickSegment(page, 'dialogue');
  await expect(page.locator('#sloppad-circle-dot')).toHaveAttribute('aria-label', /Live mode: Dialogue/);
  await expect(page.locator('#sloppad-circle-dot .sloppad-circle-dot-badge')).toHaveText('Dialogue');
});

test('silent stays independent from tool selection', async ({ page }) => {
  await openCircle(page);
  await clickSegment(page, 'silent');
  await expect(page.locator('#sloppad-circle-segment-silent')).toHaveAttribute('aria-pressed', 'true');

  await clickSegment(page, 'pointer');
  await expect(page.locator('#sloppad-circle-segment-silent')).toHaveAttribute('aria-pressed', 'true');
  await expect(page.locator('#sloppad-circle-segment-pointer')).toHaveAttribute('aria-pressed', 'true');
});
...
test('fast stays orthogonal and surfaces LOCAL plus FAST in runtime summary', async ({ page }) => {
  ...
  await expect(page.locator('#sloppad-circle-segment-fast')).toHaveAttribute('aria-pressed', 'true');
...
test('corner placement persists across reloads', async ({ page }) => {
  ...
  await page.locator('#sloppad-circle-corner-controls [data-corner="top_left"]').click();
  await expect(page.locator('#sloppad-circle')).toHaveAttribute('data-corner', 'top_left');
...
test.describe('mobile hit targets', () => {
  ...
  test('right edge strip does not steal live, silent, or tool taps', async ({ page }) => {
    ...
    await page.locator('#sloppad-circle-dot').click();
    await expect(page.locator('#sloppad-circle')).toHaveAttribute('data-state', 'expanded');

    await page.locator('#sloppad-circle-segment-meeting').click();
    await expect(page.locator('#sloppad-circle-segment-meeting')).toHaveAttribute('aria-pressed', 'true');
    await expect(page.locator('#edge-top-models .edge-live-status')).toContainText('Meeting');

    await page.locator('#sloppad-circle-segment-silent').click();
    await expect(page.locator('#sloppad-circle-segment-silent')).toHaveAttribute('aria-pressed', 'true');

    await page.locator('#sloppad-circle-segment-fast').click();
    await expect(page.locator('#sloppad-circle-segment-fast')).toHaveAttribute('aria-pressed', 'true');

    await page.locator('#sloppad-circle-segment-ink').click();
    await expect(page.locator('#sloppad-circle-segment-ink')).toHaveAttribute('aria-pressed', 'true');
    await expect(page.locator('#sloppad-circle-dot')).toHaveAttribute('data-tool', 'ink');
