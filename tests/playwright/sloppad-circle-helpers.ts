...
export type SloppadCircleSegment =
  | 'dialogue'
  | 'meeting'
  | 'silent'
  | 'fast'
...
export function circleSegment(page: Page, segment: SloppadCircleSegment): Locator {
...
export async function openCircle(page: Page) {
  if (await page.locator('#sloppad-circle').getAttribute('data-state') === 'expanded') {
    return;
  }
  await page.evaluate(() => {
...
export async function clickCircleSegment(page: Page, segment: SloppadCircleSegment) {
  await openCircle(page);
  await page.evaluate((targetSegment) => {
    const button = document.querySelector(`#sloppad-circle-menu .sloppad-circle-segment[data-segment="${targetSegment}"]`);
    if (!(button instanceof HTMLButtonElement)) {
      throw new Error(`sloppad circle segment not found: ${targetSegment}`);
    }
    button.click();
  }, segment);
}
