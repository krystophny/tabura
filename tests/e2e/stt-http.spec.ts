...
test.describe('STT over HTTP @local-only', () => {
  ...
  test('POST WAV speech to /api/stt/transcribe returns transcript', async () => {
    const wav = await synthesizePiperWav('Slopshell end to end speech to text verification.');
  ...
  test('POST M4A speech to /api/stt/transcribe returns transcript', async () => {
    const wav = await synthesizePiperWav('Slopshell m4a normalization to stt verification.');
