...
class SlopshellFlowUiTest {
    ...
    private fun runFlow(selectors: Map<String, String>, flow: FlowDefinition) {
        ...
        requireObject(device, "slopshell_circle_dot")
    ...
    private fun assertExpectations(device: UiDevice, expect: JSONObject?) {
        ...
        expect.optString("slopshell_circle").takeIf { it.isNotBlank() }?.let {
            assertEquals(it, stateValue(device, "flow_state_slopshell_circle"))
        }
    ...
    private fun launchIntent(preconditionsJSON: String): Intent {
        return Intent(ApplicationProvider.getApplicationContext(), MainActivity::class.java).apply {
            ...
            putExtra("slopshell.flow_harness", true)
            putExtra("slopshell.flow_preconditions_json", preconditionsJSON)
        }
    }
