...
fun SlopshellCanvasWebView(
    ...
    AndroidView(
        modifier = modifier,
        factory = { context ->
            SlopshellCanvasDisplayWebView(context).apply {
                ...
                webViewClient = object : WebViewClient() {
                    override fun onPageFinished(view: WebView, url: String?) {
                        ...
                        (view as? SlopshellCanvasDisplayWebView)?.onContentRendered()
                    }
                }
            }
        },
...
private class SlopshellCanvasDisplayWebView(
    context: Context,
) : WebView(context) {
    ...
    fun setEinkDisplay(enabled: Boolean) {
        ...
        if (enabled) {
            SlopshellBooxEinkController.configureContentView(this)
            SlopshellBooxEinkController.setWebViewContrastOptimize(this, true)
            return
        }
        pendingRefresh?.let(::removeCallbacks)
        pendingRefresh = null
        SlopshellBooxEinkController.setWebViewContrastOptimize(this, false)
    }
    ...
    private fun scheduleRefresh() {
        ...
        val refresh = Runnable {
            SlopshellBooxEinkController.refreshContentView(this)
        }
