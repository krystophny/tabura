...
class FlowRunner(
    ...
    private fun tap(target: String) {
        ...
        when (target) {
            "slopshell_circle_dot" -> state.circleExpanded = !state.circleExpanded
            "slopshell_circle_segment_pointer" -> state.tool = "pointer"
            "slopshell_circle_segment_highlight" -> state.tool = "highlight"
            "slopshell_circle_segment_ink" -> state.tool = "ink"
            "slopshell_circle_segment_text_note" -> state.tool = "text_note"
            "slopshell_circle_segment_prompt" -> state.tool = "prompt"
            "slopshell_circle_segment_dialogue" -> toggleSession("dialogue")
            "slopshell_circle_segment_meeting" -> toggleSession("meeting")
            "slopshell_circle_segment_silent" -> state.silent = !state.silent
            "indicator_border" -> {
                state.session = "none"
    ...
    private fun assertExpectations(expect: FlowExpectations?) {
        ...
        expect.slopshellCircle?.let { check(snapshot.slopshellCircle == it) { "expected slopshell_circle $it, got ${snapshot.slopshellCircle}" } }
    ...
    private fun snapshot(): FlowSnapshot {
        ...
        return FlowSnapshot(
            ...
            session = state.session,
            silent = state.silent,
            slopshellCircle = if (state.circleExpanded) "expanded" else "collapsed",
