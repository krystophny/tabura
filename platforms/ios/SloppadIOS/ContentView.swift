...
struct ContentView: View {
    @StateObject private var model = SloppadAppModel()
    ...
    private var workspacePicker: some View {
        HStack {
            ...
            .buttonStyle(.bordered)
            Spacer()
            Toggle("Ink asks Sloppad", isOn: $model.inkRequestsResponse)
                .toggleStyle(.switch)
                .labelsHidden()
        }
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Picker("Surface", selection: Binding(
                    get: { SloppadCompanionIdleSurface(raw: model.companionIdleSurface) },
                    set: { surface in
                        Task { await model.setDialogueIdleSurface(surface) }
                    }
                )) {
                    Text("Robot").tag(SloppadCompanionIdleSurface.robot)
                    Text("Black").tag(SloppadCompanionIdleSurface.black)
                }
    ...
    private var canvasPanel: some View {
        ZStack(alignment: .topTrailing) {
            SloppadCanvasWebView(html: model.canvas.html, baseURL: URL(string: model.serverURLString))
                .frame(minHeight: 260)
            ...
            SloppadInkCaptureView { strokes in
...
struct SloppadFlowHarnessPreconditions {
    var tool = "pointer"
    var session = "none"
    var silent = false
    var indicatorState = ""
...
private struct SloppadFlowHarnessState {
...
func parseSloppadFlowHarnessPreconditions(_ raw: String?) -> SloppadFlowHarnessPreconditions {
    guard
        let raw,
        let data = raw.data(using: .utf8),
        let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
    else {
        return SloppadFlowHarnessPreconditions()
    }
    return SloppadFlowHarnessPreconditions(
...
private func makeSloppadFlowHarnessState(preconditions: SloppadFlowHarnessPreconditions) -> SloppadFlowHarnessState {
    SloppadFlowHarnessState(
...
struct SloppadFlowHarnessRootView: View {
    @State private var state: SloppadFlowHarnessState

    init(preconditions: SloppadFlowHarnessPreconditions) {
        _state = State(initialValue: makeSloppadFlowHarnessState(preconditions: preconditions))
    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(spacing: 16) {
                    ...
                    .buttonStyle(.plain)
                    .accessibilityIdentifier("sloppad_circle_dot")
                ...
                if state.circleExpanded {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 100), spacing: 8)], spacing: 8) {
                        flowHarnessSegment(id: "sloppad_circle_pointer", label: "Pointer") {
                            state.activeTool = "pointer"
                        }
                        flowHarnessSegment(id: "sloppad_circle_highlight", label: "Highlight") {
                            state.activeTool = "highlight"
                        }
                        flowHarnessSegment(id: "sloppad_circle_ink", label: "Ink") {
                            state.activeTool = "ink"
                        }
                        flowHarnessSegment(id: "sloppad_circle_text_note", label: "Text") {
                            state.activeTool = "text_note"
                        }
                        flowHarnessSegment(id: "sloppad_circle_prompt", label: "Prompt") {
                            state.activeTool = "prompt"
                        }
                        flowHarnessSegment(id: "sloppad_circle_dialogue", label: "Dialogue") {
                            state.session = state.session == "dialogue" ? "none" : "dialogue"
                            state.indicatorOverride = ""
                        }
                        flowHarnessSegment(id: "sloppad_circle_meeting", label: "Meeting") {
                            state.session = state.session == "meeting" ? "none" : "meeting"
                            state.indicatorOverride = ""
                        }
                        flowHarnessSegment(id: "sloppad_circle_silent", label: "Silent") {
                            state.silent.toggle()
                        }
                    }
                }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    flowHarnessValue(state.sloppadCircle, id: "flow_state_sloppad_circle")
