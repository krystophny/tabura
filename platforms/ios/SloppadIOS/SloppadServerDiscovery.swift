...
final class SloppadServerDiscovery: NSObject, ObservableObject, NetServiceBrowserDelegate, NetServiceDelegate {
    @Published private(set) var servers: [SloppadDiscoveredServer] = []

    private var browser: NetServiceBrowser?
    private var discovered: [String: SloppadDiscoveredServer] = [:]
    ...
    func start() {
        ...
        browser.searchForServices(ofType: "_sloppad._tcp.", inDomain: "local.")
    }
    ...
    func netServiceDidResolveAddress(_ sender: NetService) {
        ...
        discovered[id] = SloppadDiscoveredServer(id: id, name: sender.name, host: cleanHost, port: sender.port)
        publishServers()
    }
