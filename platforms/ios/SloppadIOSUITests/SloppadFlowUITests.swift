...
final class SloppadFlowUITests: XCTestCase {
    ...
    func testSharedFlowsExecuteOnIOSHarness() throws {
        ...
        for flow in bundle.flows {
            ...
            app.launchArguments = ["-SloppadFlowHarness"]
            app.launchEnvironment["SLOPPAD_FLOW_PRECONDITIONS_JSON"] = flow.preconditions?.jsonString() ?? "{}"
            app.launch()
            XCTAssertTrue(element("sloppad_circle_dot", in: app).waitForExistence(timeout: 5), flow.name)
    ...
    private func assert(expectations: FlowExpectations?, app: XCUIApplication) {
        ...
        if let sloppadCircle = expectations.sloppadCircle {
            XCTAssertEqual(stateValue("flow_state_sloppad_circle", app: app), sloppadCircle)
        }
    ...
    private func loadFixtureBundle() throws -> FlowFixtureBundle {
        ...
        guard let url = bundle.url(forResource: "flow-fixtures", withExtension: "json") else {
            throw NSError(domain: "SloppadFlowUITests", code: 1, userInfo: [NSLocalizedDescriptionKey: "missing flow-fixtures.json"])
        }
