...
final class SlopshellServerDiscovery: NSObject, ObservableObject, NetServiceBrowserDelegate, NetServiceDelegate {
    @Published private(set) var servers: [SlopshellDiscoveredServer] = []

    private var browser: NetServiceBrowser?
    private var discovered: [String: SlopshellDiscoveredServer] = [:]
    ...
    func start() {
        ...
        browser.searchForServices(ofType: "_slopshell._tcp.", inDomain: "local.")
    }
    ...
    func netServiceDidResolveAddress(_ sender: NetService) {
        ...
        discovered[id] = SlopshellDiscoveredServer(id: id, name: sender.name, host: cleanHost, port: sender.port)
        publishServers()
    }
