...
struct ContentView: View {
    @StateObject private var model = SlopshellAppModel()
    ...
    private var workspacePicker: some View {
        HStack {
            ...
            .buttonStyle(.bordered)
            Spacer()
            Toggle("Ink asks Slopshell", isOn: $model.inkRequestsResponse)
                .toggleStyle(.switch)
                .labelsHidden()
        }
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Picker("Surface", selection: Binding(
                    get: { SlopshellCompanionIdleSurface(raw: model.companionIdleSurface) },
                    set: { surface in
                        Task { await model.setDialogueIdleSurface(surface) }
                    }
                )) {
                    Text("Robot").tag(SlopshellCompanionIdleSurface.robot)
                    Text("Black").tag(SlopshellCompanionIdleSurface.black)
                }
    ...
    private var canvasPanel: some View {
        ZStack(alignment: .topTrailing) {
            SlopshellCanvasWebView(html: model.canvas.html, baseURL: URL(string: model.serverURLString))
                .frame(minHeight: 260)
            ...
            SlopshellInkCaptureView { strokes in
...
struct SlopshellFlowHarnessPreconditions {
    var tool = "pointer"
    var session = "none"
    var silent = false
    var indicatorState = ""
...
private struct SlopshellFlowHarnessState {
...
func parseSlopshellFlowHarnessPreconditions(_ raw: String?) -> SlopshellFlowHarnessPreconditions {
    guard
        let raw,
        let data = raw.data(using: .utf8),
        let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
    else {
        return SlopshellFlowHarnessPreconditions()
    }
    return SlopshellFlowHarnessPreconditions(
...
private func makeSlopshellFlowHarnessState(preconditions: SlopshellFlowHarnessPreconditions) -> SlopshellFlowHarnessState {
    SlopshellFlowHarnessState(
...
struct SlopshellFlowHarnessRootView: View {
    @State private var state: SlopshellFlowHarnessState

    init(preconditions: SlopshellFlowHarnessPreconditions) {
        _state = State(initialValue: makeSlopshellFlowHarnessState(preconditions: preconditions))
    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(spacing: 16) {
                    ...
                    .buttonStyle(.plain)
                    .accessibilityIdentifier("slopshell_circle_dot")
                ...
                if state.circleExpanded {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 100), spacing: 8)], spacing: 8) {
                        flowHarnessSegment(id: "slopshell_circle_pointer", label: "Pointer") {
                            state.activeTool = "pointer"
                        }
                        flowHarnessSegment(id: "slopshell_circle_highlight", label: "Highlight") {
                            state.activeTool = "highlight"
                        }
                        flowHarnessSegment(id: "slopshell_circle_ink", label: "Ink") {
                            state.activeTool = "ink"
                        }
                        flowHarnessSegment(id: "slopshell_circle_text_note", label: "Text") {
                            state.activeTool = "text_note"
                        }
                        flowHarnessSegment(id: "slopshell_circle_prompt", label: "Prompt") {
                            state.activeTool = "prompt"
                        }
                        flowHarnessSegment(id: "slopshell_circle_dialogue", label: "Dialogue") {
                            state.session = state.session == "dialogue" ? "none" : "dialogue"
                            state.indicatorOverride = ""
                        }
                        flowHarnessSegment(id: "slopshell_circle_meeting", label: "Meeting") {
                            state.session = state.session == "meeting" ? "none" : "meeting"
                            state.indicatorOverride = ""
                        }
                        flowHarnessSegment(id: "slopshell_circle_silent", label: "Silent") {
                            state.silent.toggle()
                        }
                    }
                }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    flowHarnessValue(state.slopshellCircle, id: "flow_state_slopshell_circle")
